...
public class GeneticAlgorithm {
    ...
    private double hybrydizationFactor = 0.5;//współczynnik krzyżowania
    private double mutationFactor = 0.05;//współczynnik mutacji
    ...
    public GeneticAlgorithm(Population population, double hybrydizationFactor, double mutationFactor, int iterationNumber) {
        ...
        this.ic = new IndividualCounter();
    }

    /**
     * Metoda ta realizuje operację ewolucji na podanej populacji z
     * wykorzystaniem pozostałych metod zawartych w tej klasie.
     *
     * @param population populacja na której przeprowadzić operację ewolucji
     */
    public Population evolve(Population populationToEvolve) {
        //tournamentSelection(population);
        //mutate(population.getIndividualsPopulation().get(0));
        //crossover(population.getIndividualsPopulation().get(0), population.getIndividualsPopulation().get(1));

        Population newPopulation = new Population(populationToEvolve.individualsPopulationSize());
        ...
        for (int i = 0; i < populationToEvolve.individualsPopulationSize(); i++) {
            Individual individual1 = populationToEvolve.getIndividualsPopulation().get(parentPopulationIndividualsNumber[r.nextInt(parentPopulationIndividualsNumber.length)]);
            Individual individual2 = populationToEvolve.getIndividualsPopulation().get(parentPopulationIndividualsNumber[r.nextInt(parentPopulationIndividualsNumber.length)]);
    ...
    /**
     * Metoda ta implementuje krzyzowanie dwóch wybranych osobników danej
     * populacji.
     *
     * @param i1 osobnik od którego pobrać część genów do krzyżowania
     * @param i2 osobnik od którego pobrać część genów do krzyżowania
     * @return nowy osobnik powstały po skrzyżowaniu dwóch podanych osobników
     */
    private Individual crossover(Individual individual1, Individual individual2) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (r.nextDouble() <= hybrydizationFactor) {
                switch (i) {
                    case 0://sklep, drzwi, stoły, krzesła
                        ...
                        Shop s = new Shop(individual1.getShop().getX(), individual1.getShop().getY(), individual1.getShop().getLengthX(), individual1.getShop().getLengthY(), individual1.getShop().getPrice(), individual1.getShop().getImageID());

                        solutionIndividual.setShop(s);

                        Door d = new Door(individual1.getDoor().getX(), individual1.getDoor().getY(), individual1.getDoor().getLength(), individual1.getDoor().getPosition(), individual1.getDoor().getPrice(), individual1.getDoor().getImageID());
                        ...
                        break;
                    case 1://okna - wielkość okna

                        switch (r.nextInt(2)) {
                            case 0://wielkość okna
                                Window w = null;
                                ...
                                break;
                            case 1://odległość między oknami

                                int windowsType = r.nextInt(ic.getWindows().length);

                                ic.setWindows(solutionIndividual, individual1.getDistanceBetweenWindows(), ic.getWindows()[windowsType]);
                    ...
                    case 2://oswietlenie
                        ...
                        switch (r.nextInt(2)) {
                            case 0://odległosc X
                                ...
                                ic.setLighting(solutionIndividual, individual1.getDistanceBetweenLampsX(), distanceBetweenLampsY);
                                break;
                            case 1://odległosc Y
                                double distanceBetweenLampsX = r.nextDouble() * 2 + 1;
                                ic.setLighting(solutionIndividual, distanceBetweenLampsX, individual1.getDistanceBetweenLampsY());
                                break;
                        }
                        break;
                }
                ...
                switch (i) {
                    case 0://sklep, drzwi, stoły, krzesła
                        ...
                        Shop s = new Shop(individual2.getShop().getX(), individual2.getShop().getY(), individual2.getShop().getLengthX(), individual2.getShop().getLengthY(), individual2.getShop().getPrice(), individual2.getShop().getImageID());

                        solutionIndividual.setShop(s);

                        Door d = new Door(individual2.getDoor().getX(), individual2.getDoor().getY(), individual2.getDoor().getLength(), individual2.getDoor().getPosition(), individual2.getDoor().getPrice(), individual2.getDoor().getImageID());
                    ...
                    case 1://okna - wielkość okna
                        ...
                        switch (r.nextInt(2)) {
                            ...
                            case 1://odległość między oknami
                                ...
                                ic.setWindows(solutionIndividual, individual2.getDistanceBetweenWindows(), ic.getWindows()[windowsType]);
                    ...
                    case 2://oswietlenie
                        ...
                        switch (r.nextInt(2)) {
                            case 0://odległosc X
                                double distanceBetweenLampsY = r.nextDouble() * 2 + 1;
                                ic.setLighting(solutionIndividual, individual2.getDistanceBetweenLampsX(), distanceBetweenLampsY);
                                break;
                            case 1://odległosc Y
                                double distanceBetweenLampsX = r.nextDouble() * 2 + 1;
                                ic.setLighting(solutionIndividual, distanceBetweenLampsX, individual2.getDistanceBetweenLampsY());
                                break;
                        }
    ...
    /**
     * Metoda ta realizuje operacje mutacji losowo wybranego genu danej
     * populacji.
     *
     * @param i osobnik na którym przeprowadzić mutację
     */
    ...
    private void rouletteSelection(Population population) {
        ...
        for (int i = 0; i < population.getIndividualsPopulation().size(); i++) {
            rankingTab[i] = population.getIndividualsPopulation().get(i).getComfortPoints() * 100 / comfortPointsSumOfPopulation;
        }
